...

{
    ...
    if (node->item & 1)
    {
        // 홀수일 경우 임시 변수에 값을 할당해준다.
        hap = node->item;
    }

    // 현재 홀수인 값과 왼쪽 노드와 오른쪽 노드를 재귀적으로 탐색해서 홀수인 값들을 더해준다.
    return hap + sumOfOddNodes(node->left) + sumOfOddNodes(node->right);
}
