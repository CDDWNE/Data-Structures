...
{
    // 문제에 있는 조건 중 트리가 둘 다 비어있는 경우 같은 트리로 판단한다는 것을 탈출 조건으로 이용
    if (tree1 == NULL && tree2 == NULL)
    {
        return 1;
    }
    // 둘 중 하나의 트리만 빈 경우는 같은 트리가 아님으로 0을 리턴
    if (tree1 == NULL || tree2 == NULL)
    {
        return 0;
    }
    // 둘의 값이 같지 않은 경우에 같은 트리가 아님으로 0을 리턴
    if (tree1->item != tree2->item)
    {
        return 0;
    }
    // 재귀적으로 왼쪽 자식들을 비교하고, 오른쪽 자식들을 비교한다.
    // 마지막 노드들이 동시에 비어있다면 1을 리턴하고 중간에 두 트리의 값이 틀리지 않았다면
    // && 연산자를 통해서 1을 계속해서 리턴한다.
    // 그렇기에 마지막까지 값이 다르지 않았고 동시에 마지막 노드라면
    // 1을 리턴해서 같은 트리임을 확인한다.
