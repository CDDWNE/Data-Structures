...
{
    // 마지막에 왼쪽 자식의 깊이와 오른쪽 자식의 깊이 중 깊은 값에 +1을 해줘서 리턴하기 떄문에
    // 루트 노드만 있는 경우까지 생각해서 값이 존재하지 않는다면 -1을 리턴해준다.
    if (node == NULL)
    {
        return -1;
    }
    // 재귀적으로 현재 노드의 왼쪽만, 혹은 오른쪽만 탐색해서 그 깊이를 구한다.
    int maxleft = maxHeight(node->left);
    int maxright = maxHeight(node->right);

    // 왼쪽 노드의 깊이와 오른쪽 노드의 깊이 중 더 큰 값을 반환하는데
    // 여기에 1을 더하는 이유는 현재 노드의 높이를 포함시키기 위함이다.
    if (maxleft > maxright)
    {
        return maxleft + 1;
    }
